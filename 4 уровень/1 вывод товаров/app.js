...
let html_product = document.querySelector('.products')
...
function showCategory(category) {
    ...
    for (const i in items) {
        html_product.insertAdjacentHTML('afterbegin', ` <div class="product">
            <div>${items[i].name}</div>
                <img src="${items[i].imageUrl}" alt="">
                <div>${items[i].price}</div>
                <a href="https://example.com/producs/${items[i].id}">Подробнее</a>
            </div>`)
